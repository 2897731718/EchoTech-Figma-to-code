...
```bash
pnpm add -g figma-to-code   # 全局安装，推荐
# 或
pnpm add figma-to-code      # 项目内安装
```
...
```bash
# 通用模板
npx figma-to-code init

# 指定组件库（目前支持 your-ui-lib）
npx figma-to-code init --ui=your-ui-lib
```
...
详细技术方案见 [docs/architecture.md](./docs/architecture.md#i18n-变量支持)。
...
升级 figma-to-code 后重新执行 `npx figma-to-code init` 可获取最新版本（已存在时不覆盖）。
...
template/
├── commands/
│   ├── Figma-context.md              # 通用初始化模板
│   └── Figma-context-{uiLib}.md      # 各产品线专用模板（放在 figma-base-{uiLib}/ 下）
├── figma-base-your-ui-lib/
│   └── Figma-context-your-ui-lib.md       # CustomUI 模板
└── figma-base-react/
    └── Figma-context-react.md         # React 模板
```

模板格式参考 `template/commands/Figma-context.md`，重点填写：
1. 包名/前缀
2. UnoCSS 单位说明
3. Token 映射
